...
struct Labaibacao2nguoiflag {
    int giatribai;   // 1-13 (ách đến già) A -> K
    int kyhieubai;   // 0-3  (4 ký hiệu cơ/rô/chuồng/bích)
};

// Phân loại bài
enum Loaibai2nguoiflag {
    Bu2nguoiflag, Nut2nguoiflag, Bacao2nguoiflag
};

bool kiemtrabacao2nguoiflag(const vector<Labaibacao2nguoiflag>& bai){
...
Loaibai2nguoiflag loaibai2nguoi(const vector<Labaibacao2nguoiflag>& bai) {
    if (kiemtrabacao2nguoiflag(bai)) return Bacao2nguoiflag;
    int diem = 0;
    for (auto& b : bai) {
    ...
    diem %= 10;
    if (diem == 0) return Bu2nguoiflag;
    return Nut2nguoiflag;
}

void inlabai_bacao_oss_2nguoiflag(const Labaibacao2nguoiflag& b, const int diemtungla) {
    ostringstream oss;
...
vector<Labaibacao2nguoiflag> taobobai52la_bacao_2nguoiflag() {
    vector<Labaibacao2nguoiflag> bobai;
    for (int c = 0; c < 4; c++) {
        for (int g = 1; g <= 13; g++) {
            bobai.push_back({g, c}); 
        }
...
void xaobaibacao2nguoiflag(vector<Labaibacao2nguoiflag>& bobai) {
    random_device rd;
    mt19937 gen(rd());
    shuffle(bobai.begin(), bobai.end(), gen);
}

Labaibacao2nguoiflag rutbaibacao2nguoi(vector<Labaibacao2nguoiflag>& bobai) {
    Labaibacao2nguoiflag b = bobai.back();
    bobai.pop_back();
    return b;
}

string tenbaibacao2nguoiflag(const Labaibacao2nguoiflag& b) {
    string ten;
...
void game_bacao2nguoi_flag() {
    ...
    vector<Labaibacao2nguoiflag> bobai = taobobai52la_bacao_2nguoiflag();
    xaobaibacao2nguoiflag(bobai);
    vector<Labaibacao2nguoiflag> bainguoi1, bainguoi2;
    // Phát mỗi người 3 lá
    for (int i = 0; i < 3; i++) {
        bainguoi1.push_back(rutbaibacao2nguoi(bobai));
        bainguoi2.push_back(rutbaibacao2nguoi(bobai));
    }
    auto tinhdiembaicao = [](vector<Labaibacao2nguoiflag>& bai) {
        int tong = 0;
        for (auto& b : bai) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        cout << endl;
        inlabai_bacao_oss_2nguoiflag(bainguoi1[i], diemtungla);
        cout << endl;
        if(i < 2) getchar();
    }
    cout << endl;
    if(diemnguoi1 == 0){
        if(kiemtrabacao2nguoiflag(bainguoi1)) cout << "\tTổng số nút của bạn: " << YELLOW << "3 Cào" << RESET << endl;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        cout << endl;
        inlabai_bacao_oss_2nguoiflag(bainguoi2[i], diemtungla);
        cout << endl;
        if(i < 2) getchar();
    }
    cout << endl;
    if(diemnguoi2 == 0){
        if(kiemtrabacao2nguoiflag(bainguoi2)) cout << "\tTổng số nút của bạn: " << YELLOW << "3 Cào" << RESET << endl;
        else cout << "\tTổng số nút của bạn: " << YELLOW << "Bù" << RESET << endl;
    }
    else cout << "\tTổng số nút của bạn: " << YELLOW << diemnguoi2 << RESET << " nút" << endl;
    cout << endl;

    Loaibai2nguoiflag loaibainguoi1 = loaibai2nguoi(bainguoi1); 
    Loaibai2nguoiflag loaibainguoi2 = loaibai2nguoi(bainguoi2);

    string ketqua;
    if (loaibainguoi1 == Bacao2nguoiflag && loaibainguoi2 != Bacao2nguoiflag) ketqua = "Thắng";
    else if (loaibainguoi1 != Bacao2nguoiflag && loaibainguoi2 == Bacao2nguoiflag) ketqua = "Thua";
    else if (loaibainguoi1 == Bacao2nguoiflag && loaibainguoi2 == Bacao2nguoiflag) ketqua = "Hòa";
    else {
