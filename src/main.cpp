...
ThongTinPtr khoitaotaikhoan(ThongTinNguoiChoi& thongtinnguoichoi){
    ...
    tt->mauchude = thongtinnguoichoi.mauchude;
    return tt;
}
...
int phiencuoicung(string& tentaikhoan){
    ...
    if(!data.contains(tentaikhoan)) 
        return 0;
    if(!data[tentaikhoan].is_array()) 
        return 0;
    if(data[tentaikhoan].empty()) 
        return 0;
...
int tinhlevelnguoichoi(ThongTinPtr& nguoichoi){
    ...
    if(level > levelcaonhat) 
        level = levelcaonhat;
    if(level < levelthapnhat) 
        level = levelthapnhat;
    return level;
}
...
string tinhranknguoichoi(int level){
    if(level <= 2) 
        return ORANGE "Đồng nát" RESET;
    if(level <= 4) 
        return GRAY "Bạc rách" RESET;
    if(level <= 6) 
        return YELLOW "Vàng cứt" RESET;
    if(level <= 8) 
        return BLUE "Bạch kim" RESET;
    return CYAN "Kim cương" RESET;
}

// đổi màu chủ đề
void doimauchude(const string mau, int thutumau){
    if(mau.empty()) mauchude = CYAN;
    else mauchude = mau;
...
void inbanner(const string tenbanner, const string mau){
    stringstream ss(tenbanner);
    string line;
    cout << endl;
    while (getline(ss, line)) {
        ingiuamanhinh(line, mau);
        cout << endl;
    }
}
...
// chuyển đổi bật tắt AI (gợi ý kết quả 1 số game)
void chuyendoitrangthaiAIgoiy(bool& trangthaiAIgoiy){
    if(!trangthaiAIgoiy) 
        trangthaiAIgoiy = true;
    else
        trangthaiAIgoiy = false;
}

// chuyển đổi bật tắt trỏ chuột
void chuyendoitrangthaitrochuot(bool& trangthaicontrochuot){
    if(!trangthaicontrochuot)
        trangthaicontrochuot = true;
    else 
        trangthaicontrochuot = false;
}

// bật tắt âm thanh
void chuyendoitrangthaiamthanh(bool& trangthaiamthanh){
    if(!trangthaiamthanh) 
        trangthaiamthanh = true;
    else 
        trangthaiamthanh = false;
}
...
bool xacthucthongtin(DanhSachNguoiChoi& danhsachnguoichoi, 
    ...
    while(tt != NULL){
        if(thongtin == 1){   // 1. kiem tra ten tai khoan
            if(tt->tentaikhoan == thongtinnguoichoi.tentaikhoan) 
                return true;
...
bool dangkytaikhoan(DanhSachNguoiChoi& danhsachnguoichoi, ThongTinNguoiChoi& thongtinnguoichoi){
    ...
    if(solansaima6so == solansaitoida) 
        return false;
    // nhap so du 
...
bool dangnhaptaikhoan(DanhSachNguoiChoi& danhsachnguoichoi, ThongTinNguoiChoi& thongtinnguoichoi){
    ...
    if(solansaimatkhau == solansaitoida) 
        return false;
    cout << endl;
    if (thongtinnguoichoi.phanquyen == Admin) {
...
bool quenmatkhau(DanhSachNguoiChoi& danhsachnguoichoi, ThongTinNguoiChoi& thongtinnguoichoi){
    ...
    if(solansaimakhoa6so == solansaitoida) 
        return false;
    ...
    if(solansaimatkhaumoi == solansaitoida) 
        return false;
...
vector<ThongTinNguoiChoi> taithongtinnguoichoi(DanhSachNguoiChoi& danhsachnguoichoi){
    ...
    for(auto& i : data){
        ...
        thongtinnguoichoi.mauchude      = i["mauchude"];
...
void luulichsujson(string& tentaikhoan, LichSu& lichsu){
    ...
    if(filein.is_open()){
        if(!kiemtrajsontrong(dd_danhsachlichsu)) 
            filein >> data;
        filein.close();
    }
    if(!data.contains(tentaikhoan)){
            ...
            if(chon == 0){
                ...
                if(xacthucdangnhapdangky(danhsachnguoichoi, thongtinnguoichoi, 2)){
                    ...
                    if(thongtinnguoichoi.phanquyen == Nguoichoi){
                        mauchude = mangmau[nguoichoi->mauchude];
...
void sanh_caidat(DanhSachNguoiChoi& danhsachnguoichoi, ThongTinPtr& nguoichoi){
    ...
    int thutumau = nguoichoi->mauchude;
    ...
    while(1){
        ...
        if(phim == 72) 
            ...
            if(chon == 0){
                ...
                // đã thêm AI vào game -> sau khi nhập A, B AI sẽ đoán số trong khoảng đó 
    ...
    while(1){
        ...
        if(phim == 72) 
            ...
            if(chon == 0){
                ...
                //ai_goiy(nguoichoi->tentaikhoan, "taixiu");
...
void chosaukhinhapthanhcong(int sogiay) {
    ...
    #ifdef _WIN32
        int delay = (sogiay * 130) / dodaithanh;
    #else 
        int delay = (sogiay * 160) / dodaithanh;
    #endif
...
bool nhaptiencuoc(int& tiencuoc, ThongTinPtr &nguoichoi){
    ...
    do {
        ...
        if(cin.fail()){
            ...
            cout << YELLOW << "\t\t(!) Tiền cược phải lớn hơn hoặc bằng " 
